...
export class MacroService {
  ...
  public getMacrosFromDB(date: string) {
    return this.http.get<HTTPResponse>(`${this.url}/macros/date`, { params: { date } }).pipe(
      tap(({ status, data }) => {
        ...
        const { calories, carbs, protein, fat, date } = data;
  ...
  public updateMacros(weight: number, foodName: string, date: string) {
    const food: Entry | undefined = this.entryService.foodEntries().find((entry) => entry.name === foodName);
    if (!food) {
      return;
    }

    const calculatedMacros: Partial<Macros> =
      food['servingMeasurement'] === 'one' ? Utils.calculateMacros(1, food) : Utils.calculateMacros(weight, food);
